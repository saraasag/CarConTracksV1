...
let CreateRentalPage = (props) => {
  ...
  const create = async (value, carRental) => {
    if (carRental == null) {
    ...
    // try {
    await carRental.methods.createRental(value.customerName, value.customerAge, value.licenseID, value.carID).send({from: address, value: '1000000000000000000'});
    // } catch (err) {
  ...
  let handleSubmit = async () => {
    ...
    if (state.verifier != null) {
      // create record
      let success = await create({...state}, carRental);
      if (success) {
