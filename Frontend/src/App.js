...
function App() {
  ...
  const addCar = (newCar) => {
    ...
    localStorage.setItem('car_list', JSON.stringify([...carList, newCar], 0, 2));
  ...
  const registerUser = (data) => {
    ...
    if (match) setAlert('duplicate email');
      ...
      localStorage.setItem('user_list', JSON.stringify([...carList, newUser], 0, 2));
  ...
  return (
    <div style={{ marginLeft: '30%', marginTop: '5%', width: '100%' }}>
      ...
      <h1>
        Welcome
        {' '}
        {user ? user.name : 'to Distributed Car Rentals'}
        !
        {' '}
      ...
      <Button
        color="primary"
        outline
        onClick={() => { setModal(user ? 'new_car' : 'user'); }}
      >
